...
with b1:
    if st.button("🏨 Hôtels", use_container_width=True, key="nav_hotel_veh"):
        st.switch_page("app.py")
with b2:
    if st.button("✈️ Compagnies Aériennes", use_container_width=True, key="nav_cie_veh"):
        st.switch_page("pages/1_Compagnies_Aeriennes.py")
with b3:
    if st.button("🚗 Loueurs de Véhicules", use_container_width=True, key="nav_voiture_veh"):
        st.switch_page("pages/2_Loueurs_Vehicules.py")
with b4:
    if st.button("📖 Blog", use_container_width=True, key="nav_blog_veh"):
